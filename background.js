...
let kotlin2 = [
  ...
  },
  {
    ...
    "code2": undefined,
    "choices": [
      "1. Option 1<br>```<br>var price: Int = 0<br> public get()<br> private set<br>```",
      "2. Option 2<br>```<br>var price: Int = 0<br> private set<br>```",
      "3. Option 3<br>```<br>var price: Int = 0<br> val set<br>```",
      "4. Option 4<br>```<br>val price: Int=0<br>```"
    ],
    "answer": "2. Option 2<br>```<br>var price: Int = 0<br> private set<br>```"
  },
  {
    "id": 69,
...
let AndroidBasics = [
	...
	},
	{
		...
		text: undefined,
		code2: '@Composable<br>fun MyComposable() { <br>    val context = LocalContext.current<br>    val receiver = object : BroadcastReceiver() {<br>        override fun onReceive(context: Context?, intent: Intent?) {<br>            // Handle the broadcast<br>        }<br>    }<br>    LaunchedEffect(key1 = receiver) { <br>        context.registerReceiver(receiver, IntentFilter("my_custom_action"))<br>    }<br>    // ... other composables<br>}<br>',
		choices: [
	...
	},
	{
		...
		text: undefined,
		code2: "@Composable<br>fun MyComposable() { <br>    val context = LocalContext.current<br>    val notificationManager = context.getSystemService(NotificationManager::class.java)<br>    // ... create notification builder<br>    notificationManager.notify(1, notificationBuilder.build())<br>}<br>",
		choices: [
	...
	{
		id: 70,
		title: "Q70. What does the following code do? <br><br>```kotlin<br>val alarmManager = getSystemService(AlarmManager::class.java)<br>val intent = Intent(this, MyReceiver::class.java)<br>val pendingIntent = PendingIntent.getBroadcast(this, 0, intent, 0)<br>alarmManager.set(AlarmManager.RTC_WAKEUP, triggerTimeInMillis, pendingIntent)<br>```",
		text: undefined,
		code2: undefined,
		choices: [
	...
	{
		id: 81,
		title: "Q81. What does the following code do? <br><br>```kotlin<br>val intent = Intent(this, MyService::class.java)<br>startService(intent)<br>```",
		text: undefined,
		code2: undefined,
		choices: [
	...
	{
		id: 93,
		title: "Q93. What does the following code do? <br><br>```kotlin<br>val prefs = getSharedPreferences('my_prefs', MODE_PRIVATE)<br>val editor = prefs.edit()<br>editor.putString('username', 'John Doe')<br>editor.apply()<br>```",
		text: undefined,
		code2: undefined,
		choices: [
	...
	{
		id: 100,
		title: 'Q100. What does the following code do? <br><br>```kotlin<br>val client = OkHttpClient()<br>val request = Request.Builder().url("https://www.google.com").build()<br>val response = client.newCall(request).execute()<br>```',
		text: undefined,
		code2: undefined,
		choices: [
	...
	{
		id: 108,
		title: "Q108. What does the following code do? <br><br>```kotlin<br>val permissionStatus = remember { mutableStateOf(PermissionStatus.Unknown) }<br>LaunchedEffect(key1 = Unit) { <br>    permissionStatus.value = if (ContextCompat.checkSelfPermission(context, Manifest.permission.ACCESS_FINE_LOCATION) == PackageManager.PERMISSION_GRANTED) { <br>        PermissionStatus.Granted <br>    } else { <br>        PermissionStatus.Denied <br>    }<br>}<br>```",
		text: undefined,
		code2: undefined,
		choices: [
	...
	{
		id: 116,
		title: "Q116. What does the following code do? <br><br>```kotlin<br>val workRequest = OneTimeWorkRequest.Builder(MyWorker::class.java).build()<br>WorkManager.getInstance(context).enqueue(workRequest)<br>```",
		text: undefined,
		code2: undefined,
		choices: [
	...
	{
		id: 134,
		title: "Q134. What does the following code do? <br><br>```kotlin<br>val scope = CoroutineScope(Dispatchers.IO)<br>scope.launch { <br>    // Perform a long-running network request <br>}<br>```",
		text: undefined,
		code2: undefined,
		choices: [
	...
	{
		id: 195,
		title: "Q195. What does the following code do in an Android Activity?",
		text: undefined,
		code2: 'val intent = Intent(this, SecondActivity::class.java)<br>intent.putExtra("message", "Hello from FirstActivity")<br>startActivity(intent)',
		choices: [
			"1. Starts a new activity and passes data to it.",
			"2. Creates a new background service.",
			"3. Registers a broadcast receiver.",
			"4. Displays a notification to the user.",
		],
		answer: "1. Starts a new activity and passes data to it.",
	},
	{
		id: 196,
		title: "Q196. How does the following code segment access and use a SharedPreference?",
		text: undefined,
		code2: 'val sharedPreferences = getSharedPreferences("app_prefs", Context.MODE_PRIVATE)<br>val name = sharedPreferences.getString("user_name", "")<br>// ... use the retrieved \'name\' value',
		choices: [
			"1. Reads data from a database.",
			"2. Reads data from a file.",
			"3. Reads data from a network connection.",
			"4. Reads data from a shared preference file.",
		],
		answer: "4. Reads data from a shared preference file.",
	},
	{
		id: 197,
		title: "Q197. What does the following code do in an Android Activity?",
		text: undefined,
		code2: 'val notificationManager = getSystemService(NotificationManager::class.java)<br>val notificationChannel = NotificationChannel("channel_id", "Channel Name", NotificationManager.IMPORTANCE_DEFAULT)<br>notificationManager.createNotificationChannel(notificationChannel)',
		choices: [
			"1. Displays a notification to the user.",
			"2. Creates a notification channel for future notifications.",
			"3. Registers a broadcast receiver for notifications.",
			"4. Starts a new activity to handle notifications.",
		],
		answer: "2. Creates a notification channel for future notifications.",
	},
	{
		id: 198,
		title: "Q198. What is the purpose of the following code snippet in an Android Service?",
		text: undefined,
		code2: 'val intent = Intent("com.example.app.MY_ACTION")<br>// ... set extra data in the intent<br>sendBroadcast(intent)',
		choices: [
			"1. Starts a new activity.",
			"2. Creates a notification.",
			"3. Sends a broadcast to other components.",
			"4. Reads data from a database.",
		],
		answer: "3. Sends a broadcast to other components.",
	},
	{
		id: 199,
		title: "Q199. How does the following code segment access and use a ContentResolver?",
		text: undefined,
		code2: "val contentResolver = contentResolver<br>val cursor = contentResolver.query(ContactsContract.Contacts.CONTENT_URI, null, null, null, null)<br>// ... iterate over the cursor and extract contact data",
		choices: [
			"1. Reads data from a shared preference file.",
			"2. Reads data from a file.",
			"3. Reads data from a network connection.",
			"4. Reads data from a content provider.",
		],
		answer: "4. Reads data from a content provider.",
	},
	{
		id: 200,
		title: "Q200. What does the following code do in an Android Activity?",
		text: undefined,
		code2: "val connection = getSystemService(ConnectivityManager::class.java)<br>val activeNetworkInfo = connection.activeNetworkInfo<br>if (activeNetworkInfo != null && activeNetworkInfo.isConnected) {<br>    // ... perform network operations<br>}",
		choices: [
			"1. Connects to a database.",
			"2. Checks if the device has an internet connection.",
			"3. Starts a new background service.",
			"4. Creates a notification channel.",
		],
		answer: "2. Checks if the device has an internet connection.",
	},
	{
		id: 201,
		title: "Q201. What is the purpose of the following code snippet in an Android Activity?",
		text: undefined,
		code2: "val locationManager = getSystemService(LocationManager::class.java)<br>val locationListener = object : LocationListener {<br>    override fun onLocationChanged(location: Location) {<br>        // ... process the location information<br>    }<br>}<br>locationManager.requestLocationUpdates(LocationManager.GPS_PROVIDER, 0L, 0f, locationListener)",
		choices: [
			"1. Reads data from a file.",
			"2. Creates a notification.",
			"3. Starts a new activity.",
			"4. Requests location updates from the device.",
		],
		answer: "4. Requests location updates from the device.",
	},
	{
		id: 202,
		title: "Q202. What does the following code do in an Android Activity?",
		text: undefined,
		code2: "val cameraIntent = Intent(MediaStore.ACTION_IMAGE_CAPTURE)<br>startActivityForResult(cameraIntent, REQUEST_IMAGE_CAPTURE)",
		choices: [
			"1. Sends an email.",
			"2. Launches the device's camera app.",
			"3. Starts a new background service.",
			"4. Reads data from a shared preference file.",
		],
		answer: "2. Launches the device's camera app.",
	},
	{
		id: 203,
		title: "Q203. What is the purpose of the following code snippet in an Android Activity?",
		text: undefined,
		code2: "val alarmManager = getSystemService(AlarmManager::class.java)<br>val intent = Intent(this, MyBroadcastReceiver::class.java)<br>val pendingIntent = PendingIntent.getBroadcast(this, 0, intent, 0)<br>alarmManager.set(AlarmManager.RTC_WAKEUP, triggerTime, pendingIntent)",
		choices: [
			"1. Creates a notification channel.",
			"2. Starts a new activity.",
			"3. Schedules an alarm to trigger a broadcast at a specific time.",
			"4. Reads data from a database.",
		],
		answer: "3. Schedules an alarm to trigger a broadcast at a specific time.",
	},
	{
		id: 204,
		title: "Q204. What does the following code do in an Android Activity?",
		text: undefined,
		code2: "val editText = findViewById<EditText>(R.id.editText)<br>val text = editText.text.toString()",
		choices: [
			"1. Reads data from a file.",
			"2. Retrieves text entered by the user in an EditText field.",
			"3. Sends a broadcast.",
			"4. Launches the device's camera app.",
		],
		answer: "2. Retrieves text entered by the user in an EditText field.",
	},
	{
		id: 205,
		title: "Q205. How does the following code segment use an AsyncTask?",
		text: undefined,
		code2: 'object : AsyncTask<String, Void, String>() {<br>    override fun doInBackground(vararg params: String?): String? {<br>        // ... perform background operation<br>        return "Result from background operation"<br>    }<br>    override fun onPostExecute(result: String?) {<br>        // ... update UI with result<br>    }<br>}.execute("Parameter for background operation")',
		choices: [
			"1. Starts a new activity.",
			"2. Creates a notification.",
			"3. Executes a task in the background and updates the UI with the result.",
			"4. Reads data from a shared preference file.",
		],
		answer: "3. Executes a task in the background and updates the UI with the result.",
	},
	{
		id: 206,
		title: "Q206. What does the following code do in an Android Activity?",
		text: undefined,
		code2: 'val textView = findViewById<TextView>(R.id.textView)<br>textView.text = "Hello World"',
		choices: [
			"1. Reads data from a file.",
			"2. Displays text in a TextView.",
			"3. Launches the device's camera app.",
			"4. Starts a new background service.",
		],
		answer: "2. Displays text in a TextView.",
	},
	{
		id: 207,
		title: "Q207. What is the purpose of the following code snippet in an Android Service?",
		text: undefined,
		code2: 'val intent = Intent("com.example.app.MY_ACTION")<br>// ... set extra data in the intent<br>startService(intent)',
		choices: [
			"1. Starts a new activity.",
			"2. Creates a notification.",
			"3. Starts a new background service.",
		...
		answer: "3. Starts a new background service.",
	},
	{
		id: 208,
		title: "Q208. What does the following code do in an Android Activity?",
		text: undefined,
		code2: "val button = findViewById<Button>(R.id.button)<br>button.setOnClickListener { view -> <br>    // ... perform action when button is clicked<br>}",
		choices: [
			"1. Reads data from a file.",
			"2. Sets a click listener for a button.",
			"3. Launches the device's camera app.",
			"4. Starts a new background service.",
		],
		answer: "2. Sets a click listener for a button.",
	},
	{
		id: 209,
		title: "Q209. What does the following code do in an Android Activity?",
		text: undefined,
		code2: "val progressBar = findViewById<ProgressBar>(R.id.progressBar)<br>progressBar.visibility = View.VISIBLE",
		choices: [
			"1. Starts a new activity.",
			"2. Creates a notification.",
			"3. Makes a progress bar visible.",
			"4. Reads data from a database.",
		],
		answer: "3. Makes a progress bar visible.",
	},
	{
		id: 210,
		title: "Q210. What is the purpose of the following code snippet in an Android Activity?",
		text: undefined,
		code2: 'val intentFilter = IntentFilter("android.net.conn.CONNECTIVITY_CHANGE")<br>registerReceiver(MyBroadcastReceiver(), intentFilter)',
		choices: [
			"1. Registers a broadcast receiver to listen for connectivity changes.",
			"2. Starts a new activity.",
			"3. Creates a notification.",
			"4. Reads data from a file.",
		],
		answer: "1. Registers a broadcast receiver to listen for connectivity changes.",
	},
	{
		id: 211,
		title: "Q211. What does the following code do in an Android Activity?",
		text: undefined,
		code2: "val view = LayoutInflater.from(this).inflate(R.layout.my_layout, null)<br>// ... use the inflated view",
		choices: [
			"1. Reads data from a file.",
			"2. Inflates a layout file to create a view.",
			"3. Starts a new activity.",
			"4. Creates a notification.",
		],
		answer: "2. Inflates a layout file to create a view.",
	},
	{
		id: 212,
		title: "Q212. How does the following code segment use a RecyclerView?",
		text: undefined,
		code2: "val recyclerView = findViewById<RecyclerView>(R.id.recyclerView)<br>val adapter = MyRecyclerViewAdapter(data)<br>recyclerView.adapter = adapter",
		choices: [
			"1. Displays a list of items in a scrollable view.",
			"2. Creates a notification channel.",
			"3. Starts a new activity.",
			"4. Reads data from a database.",
		],
		answer: "1. Displays a list of items in a scrollable view.",
	},
	{
		id: 213,
		title: "Q213. What does the following code do in an Android Activity?",
		text: undefined,
		code2: 'val toast = Toast.makeText(this, "Message", Toast.LENGTH_SHORT)<br>toast.show()',
		choices: [
			"1. Starts a new activity.",
			"2. Creates a notification.",
			"3. Displays a short-duration toast message.",
			"4. Reads data from a file.",
		],
		answer: "3. Displays a short-duration toast message.",
	},
	{
		id: 214,
		title: "Q214. What is the purpose of the following code snippet in an Android Activity?",
		text: undefined,
		code2: 'val sharedPreferences = getSharedPreferences("app_prefs", Context.MODE_PRIVATE)<br>val editor = sharedPreferences.edit()<br>editor.putString("user_name", "John Doe")<br>editor.apply()',
		choices: [
			"1. Reads data from a file.",
			"2. Writes data to a shared preference file.",
			"3. Starts a new activity.",
			"4. Creates a notification.",
		],
		answer: "2. Writes data to a shared preference file.",
	},
	{
		id: 215,
		title: "Q215. What does the following code do in an Android Activity?",
		text: undefined,
		code2: 'val imageUri = Uri.parse("android.resource://com.example.app/drawable/my_image")<br>val imageView = findViewById<ImageView>(R.id.imageView)<br>imageView.setImageURI(imageUri)',
		choices: [
			"1. Reads data from a file.",
			"2. Sets an image in an ImageView.",
			"3. Starts a new activity.",
			"4. Creates a notification.",
		],
		answer: "2. Sets an image in an ImageView.",
	},
	{
		id: 216,
		title: "Q216. How does the following code segment use a Handler?",
		text: undefined,
		code2: "val handler = Handler(Looper.getMainLooper())<br>handler.postDelayed({ <br>    // ... perform action after delay<br>}, 1000)",
		choices: [
			"1. Creates a notification channel.",
			"2. Starts a new activity.",
			"3. Executes an action after a specific delay on the main thread.",
			"4. Reads data from a database.",
		],
		answer: "3. Executes an action after a specific delay on the main thread.",
	},
	{
		id: 217,
		title: "Q217. What does the following code do in an Android Activity?",
		text: undefined,
		code2: "val view = findViewById<View>(R.id.view)<br>view.setOnClickListener { view -> <br>    // ... perform action when the view is clicked<br>}",
		choices: [
			"1. Reads data from a file.",
			"2. Sets a click listener for a view.",
			"3. Launches the device's camera app.",
			"4. Starts a new background service.",
		],
		answer: "2. Sets a click listener for a view.",
	},
	{
		id: 218,
		title: "Q218. What is the purpose of the following code snippet in an Android Service?",
		text: undefined,
		code2: 'val intent = Intent(this, MyBroadcastReceiver::class.java)<br>// ... set extra data in the intent<br>startForeground(NOTIFICATION_ID, NotificationCompat.Builder(this, CHANNEL_ID)<br>        .setContentTitle("Service Running")<br>        .setContentText("Doing something in the background")<br>        .setSmallIcon(R.drawable.ic_launcher)<br>        .build())',
		choices: [
			"1. Starts a new activity.",
			"2. Creates a notification.",
			"3. Starts a new background service.",
	...
	{
		id: 219,
		title: "Q219. What does the following code do in an Android Activity?",
		text: undefined,
		code2: "val switchButton = findViewById<Switch>(R.id.switchButton)<br>switchButton.setOnCheckedChangeListener { buttonView, isChecked -> <br>    // ... perform action based on switch state<br>}",
		choices: [
			"1. Reads data from a file.",
			"2. Sets a listener for switch state changes.",
			"3. Launches the device's camera app.",
			"4. Starts a new background service.",
		],
		answer: "2. Sets a listener for switch state changes.",
	},
	{
		id: 220,
		title: "Q220. How does the following code segment use a ViewPager?",
		text: undefined,
		code2: "val viewPager = findViewById<ViewPager>(R.id.viewPager)<br>val adapter = MyViewPagerAdapter(supportFragmentManager)<br>viewPager.adapter = adapter",
		choices: [
			"1. Displays a list of items in a scrollable view.",
			"2. Creates a notification channel.",
			"3. Allows swiping between different views.",
			"4. Reads data from a database.",
		],
		answer: "3. Allows swiping between different views.",
	},
	{
		id: 221,
		title: "Q221. What does the following code do in an Android Activity?",
		text: undefined,
		code2: 'val intent = Intent(Intent.ACTION_SEND)<br>intent.type = "text/plain"<br>intent.putExtra(Intent.EXTRA_TEXT, "Share this text")<br>startActivity(Intent.createChooser(intent, "Share using:"))',
		choices: [
			"1. Starts a new activity.",
			"2. Creates a notification.",
			"3. Allows the user to share content using various apps.",
			"4. Reads data from a file.",
		],
		answer: "3. Allows the user to share content using various apps.",
	},
	{
		id: 222,
		title: "Q222. What is the purpose of the following code snippet in an Android Service?",
		text: undefined,
		code2: 'val notificationManager = getSystemService(NotificationManager::class.java)<br>val notification = NotificationCompat.Builder(this, CHANNEL_ID)<br>        .setContentTitle("Service Running")<br>        .setContentText("Doing something in the background")<br>        .setSmallIcon(R.drawable.ic_launcher)<br>        .build()<br>notificationManager.notify(NOTIFICATION_ID, notification)',
		choices: [
			"1. Starts a new activity.",
			"2. Creates a notification and displays it.",
			"3. Starts a new background service.",
			"4. Reads data from a database.",
		],
		answer: "2. Creates a notification and displays it.",
	},
	{
		id: 223,
		title: "Q223. What does the following code do in an Android Activity?",
		text: undefined,
		code2: "val button = findViewById<Button>(R.id.button)<br>button.isEnabled = false",
		choices: [
			"1. Reads data from a file.",
			"2. Disables a button.",
			"3. Launches the device's camera app.",
		...
		answer: "2. Disables a button.",
	},
	{
		id: 224,
		title: "Q224. How does the following code segment use a Timer?",
		text: undefined,
		code2: "val timer = Timer()<br>val task = timer.schedule(object : TimerTask() {<br>    override fun run() {<br>        // ... perform action at regular intervals<br>    }<br>}, 0, 1000)",
		choices: [
			"1. Creates a notification channel.",
			"2. Starts a new activity.",
			"3. Executes an action at regular intervals.",
			"4. Reads data from a database.",
		],
		answer: "3. Executes an action at regular intervals.",
	},
	{
		id: 225,
		title: "Q225. What does the following code do in an Android Activity?",
		text: undefined,
		code2: "val linearLayout = findViewById<LinearLayout>(R.id.linearLayout)<br>linearLayout.orientation = LinearLayout.VERTICAL",
		choices: [
			"1. Reads data from a file.",
			"2. Sets the orientation of a LinearLayout to vertical.",
			"3. Launches the device's camera app.",
			"4. Starts a new background service.",
		],
		answer: "2. Sets the orientation of a LinearLayout to vertical.",
	},
	{
		id: 226,
		title: "Q226. What is the purpose of the following code snippet in an Android Service?",
		text: undefined,
		code2: 'val intentFilter = IntentFilter("android.net.wifi.WIFI_STATE_CHANGED")<br>registerReceiver(MyBroadcastReceiver(), intentFilter)',
		choices: [
			"1. Starts a new activity.",
			"2. Creates a notification.",
			"3. Registers a broadcast receiver to listen for WiFi state changes.",
			"4. Reads data from a database.",
		],
		answer: "3. Registers a broadcast receiver to listen for WiFi state changes.",
	},
	{
		id: 227,
		title: "Q227. What does the following code do in an Android Activity?",
		text: undefined,
		code2: "val imageView = findViewById<ImageView>(R.id.imageView)<br>imageView.setOnClickListener { view -> <br>    // ... perform action when the image is clicked<br>}",
		choices: [
			"1. Reads data from a file.",
			"2. Sets a click listener for an ImageView.",
			"3. Launches the device's camera app.",
			"4. Starts a new background service.",
		],
		answer: "2. Sets a click listener for an ImageView.",
	},
	{
		id: 228,
		title: "Q228. How does the following code segment use a CountDownTimer?",
		text: undefined,
		code2: "val timer = object : CountDownTimer(10000, 1000) {<br>    override fun onTick(millisUntilFinished: Long) {<br>        // ... update UI based on remaining time<br>    }<br>    override fun onFinish() {<br>        // ... perform action when timer finishes<br>    }<br>}.start()",
		choices: [
			"1. Creates a notification channel.",
			"2. Starts a new activity.",
			"3. Creates a timer that counts down and performs actions at intervals.",
			"4. Reads data from a database.",
		],
		answer: "3. Creates a timer that counts down and performs actions at intervals.",
	},
	{
		id: 229,
		title: "Q229. What does the following code do in an Android Activity?",
		text: undefined,
		code2: "val button = findViewById<Button>(R.id.button)<br>button.setBackgroundColor(Color.RED)",
		choices: [
			"1. Reads data from a file.",
			"2. Sets the background color of a button.",
			"3. Launches the device's camera app.",
			"4. Starts a new background service.",
		],
		answer: "2. Sets the background color of a button.",
	},
	{
		id: 230,
		title: "Q230. What is the purpose of the following code snippet in an Android Service?",
		text: undefined,
		code2: 'val intentFilter = IntentFilter("android.media.ACTION_AUDIO_BECOMING_NOISY")<br>registerReceiver(MyBroadcastReceiver(), intentFilter)',
		choices: [
			"1. Starts a new activity.",
			"2. Creates a notification.",
			"3. Registers a broadcast receiver to listen for audio becoming noisy.",
			"4. Reads data from a database.",
		],
		answer: "3. Registers a broadcast receiver to listen for audio becoming noisy.",
	},
	{
		id: 231,
		title: "Q231. What does the following code do in an Android Activity?",
		text: undefined,
		code2: "val textView = findViewById<TextView>(R.id.textView)<br>textView.setTextColor(Color.BLUE)",
		choices: [
			"1. Reads data from a file.",
			"2. Sets the text color of a TextView.",
			"3. Launches the device's camera app.",
			"4. Starts a new background service.",
		],
		answer: "2. Sets the text color of a TextView.",
	},
	{
		id: 232,
		title: "Q232. How does the following code segment use a TabLayout?",
		text: undefined,
		code2: "val tabLayout = findViewById<TabLayout>(R.id.tabLayout)<br>val viewPager = findViewById<ViewPager>(R.id.viewPager)<br>val adapter = MyViewPagerAdapter(supportFragmentManager)<br>viewPager.adapter = adapter<br>tabLayout.setupWithViewPager(viewPager)",
		choices: [
			"1. Displays a list of items in a scrollable view.",
			"2. Creates a notification channel.",
			"3. Provides tabs for switching between different views in a ViewPager.",
			"4. Reads data from a database.",
		],
		answer: "3. Provides tabs for switching between different views in a ViewPager.",
	},
	{
		id: 233,
		title: "Q233. What does the following code do in an Android Activity?",
		text: undefined,
		code2: 'val intent = Intent(Intent.ACTION_VIEW)<br>intent.data = Uri.parse("https://www.example.com")<br>startActivity(intent)',
		choices: [
			"1. Starts a new activity.",
			"2. Creates a notification.",
			"3. Opens a web page in a browser.",
			"4. Reads data from a file.",
		],
		answer: "3. Opens a web page in a browser.",
	},
	{
		id: 234,
		title: "Q234. What is the purpose of the following code snippet in an Android Service?",
		text: undefined,
		code2: "val notificationManager = getSystemService(NotificationManager::class.java)<br>notificationManager.cancel(NOTIFICATION_ID)",
		choices: [
			"1. Starts a new activity.",
			"2. Creates a notification.",
			"3. Cancels a notification.",
		...
		answer: "3. Cancels a notification.",
	},
	{
		id: 235,
		title: "Q235. What does the following code do in an Android Activity?",
		text: undefined,
		code2: "val button = findViewById<Button>(R.id.button)<br>button.setTextColor(Color.WHITE)",
		choices: [
			"1. Reads data from a file.",
			"2. Sets the text color of a button.",
			"3. Launches the device's camera app.",
			"4. Starts a new background service.",
		],
		answer: "2. Sets the text color of a button.",
	},
	{
		id: 236,
		title: "Q236. How does the following code segment use a CoordinatorLayout?",
		text: undefined,
		code2: "val coordinatorLayout = findViewById<CoordinatorLayout>(R.id.coordinatorLayout)<br>val appBarLayout = findViewById<AppBarLayout>(R.id.appBarLayout)<br>coordinatorLayout.addView(appBarLayout)",
		choices: [
			"1. Displays a list of items in a scrollable view.",
			"2. Creates a notification channel.",
			"3. Provides a layout that coordinates with other views, such as an app bar.",
			"4. Reads data from a database.",
		],
		answer: "3. Provides a layout that coordinates with other views, such as an app bar.",
	},
	{
		id: 237,
		title: "Q237. What does the following code do in an Android Activity?",
		text: undefined,
		code2: 'val intent = Intent(Intent.ACTION_DIAL)<br>intent.data = Uri.parse("tel:1234567890")<br>startActivity(intent)',
		choices: [
			"1. Starts a new activity.",
			"2. Creates a notification.",
			"3. Opens the dialer with a specific phone number.",
			"4. Reads data from a file.",
		],
		answer: "3. Opens the dialer with a specific phone number.",
	},
	{
		id: 238,
		title: "Q238. What is the purpose of the following code snippet in an Android Service?",
		text: undefined,
		code2: 'val notificationManager = getSystemService(NotificationManager::class.java)<br>val notification = NotificationCompat.Builder(this, CHANNEL_ID)<br>        .setContentTitle("Service Running")<br>        .setContentText("Doing something in the background")<br>        .setSmallIcon(R.drawable.ic_launcher)<br>        .build()<br>startForeground(NOTIFICATION_ID, notification)',
		choices: [
			"1. Starts a new activity.",
			"2. Creates a notification.",
			"3. Starts a new background service and displays a notification.",
			"4. Reads data from a database.",
		],
		answer: "3. Starts a new background service and displays a notification.",
	},
	{
		id: 239,
		title: "Q239. What does the following code do in an Android Activity?",
		text: undefined,
		code2: "val textView = findViewById<TextView>(R.id.textView)<br>textView.setTextAppearance(this, R.style.TextAppearance_Widget_AppCompat_Headline)",
		choices: [
			"1. Reads data from a file.",
			"2. Sets the text appearance of a TextView.",
			"3. Launches the device's camera app.",
			"4. Starts a new background service.",
		],
		answer: "2. Sets the text appearance of a TextView.",
	},
	{
		id: 240,
		title: "Q240. How does the following code segment use a NavigationView?",
		text: undefined,
		code2: "val navigationView = findViewById<NavigationView>(R.id.navigationView)<br>navigationView.setNavigationItemSelectedListener { menuItem -> <br>    // ... handle navigation item selection<br>    true<br>}",
		choices: [
			"1. Displays a list of items in a scrollable view.",
			"2. Creates a notification channel.",
			"3. Provides a side navigation menu.",
			"4. Reads data from a database.",
		],
		answer: "3. Provides a side navigation menu.",
	},
	{
		id: 241,
		title: "Q241. What does the following code do in an Android Activity?",
		text: undefined,
		code2: 'val intent = Intent(Intent.ACTION_SENDTO)<br>intent.data = Uri.parse("mailto:john.doe@example.com")<br>intent.putExtra(Intent.EXTRA_SUBJECT, "Email Subject")<br>intent.putExtra(Intent.EXTRA_TEXT, "Email Body")<br>startActivity(intent)',
		choices: [
			"1. Starts a new activity.",
			"2. Creates a notification.",
			"3. Opens an email composer with pre-filled details.",
			"4. Reads data from a file.",
		],
		answer: "3. Opens an email composer with pre-filled details.",
	},
	{
		id: 242,
		title: "Q242. What is the purpose of the following code snippet in an Android Service?",
		text: undefined,
		code2: 'val notificationManager = getSystemService(NotificationManager::class.java)<br>notificationManager.notify(NOTIFICATION_ID, NotificationCompat.Builder(this, CHANNEL_ID)<br>        .setContentTitle("Service Running")<br>        .setContentText("Doing something in the background")<br>        .setSmallIcon(R.drawable.ic_launcher)<br>        .build())',
		choices: [
			"1. Starts a new activity.",
			"2. Creates a notification and displays it.",
			"3. Starts a new background service.",
